import heapq
from copy import deepcopy
from astar_util import add, remove, goal_check, has
from make_action import  make_pick, make_insert
def generate_actions(objects, holes,
                     gear_orientations,
                     rod_orientations):

    actions = []

    for obj, obj_type in objects.items():

        # -----------------
        # PICK ACTIONS
        # -----------------
        if obj_type == "gear":
            for orientation, cost in gear_orientations.items():
                actions.append(make_pick(obj, orientation, cost))

        elif obj_type == "rod":
            for orientation, cost in rod_orientations.items():
                actions.append(make_pick(obj, orientation, cost))

        # -----------------
        # INSERT ACTIONS
        # -----------------
    for obj, obj_type in objects.items():

        if obj_type == "rod":
            # rod → hole
            for hole in holes.keys():
                actions.append(make_insert(obj_type, hole, cost_value=3))

        elif obj_type == "gear":
            # gear → rod
            for rod, rod_type in objects.items():
                if rod_type == "rod":
                    for hole in holes.keys():
                        for orientation, cost in gear_orientations.items():
                            actions.append(make_insert(obj_type, rod, support = rod_type, grasp_mode = orientation, cost_value=cost))
    return actions

# ----------------------------
# A* Planner
# ----------------------------

def heuristic(state, goal):
    # Simple heuristic: number of missing goal predicates
    return len(goal - state)


def astar(initial_state, goal_test, actions):

    open_list = []
    heapq.heappush(open_list, (0, 0, initial_state, []))

    visited = {}

    while open_list:
        f, g, state, plan = heapq.heappop(open_list)

        if goal_test(state):
            return plan

        if state in visited and visited[state] <= g:
            continue

        visited[state] = g

        for action in actions:
            if action.applicable(state):
                next_state = action.apply(state)
                new_g = g + action.cost(state)
                new_f = new_g + heuristic(next_state, goal_test)
                heapq.heappush(open_list,
                               (new_f, new_g, next_state, plan + [action]))

    return None


def main():
    
    object_types = ["gear", "rod"]

    # These objects should be given by the environment 
    objects = {
        "gear_1": object_types[0],
        "gear_2": object_types[0],
        "rod_1": object_types[1],
        "rod_2": object_types[1]
    }

    holes = {"hole_1": "hole", "hole_2":"hole"}

    # This initial state should be given by the obervations from the environment
    # For pick() is_grapable(o), orientation(o) holding(o)
     
    initial_state = frozenset({
        ("is_graspable", "gear_1"),
        ("orientation", "gear_1"),
        ("is_graspable", "gear_2"),
        ("holding", None),
    })
    gear_orientations = {
        "bottom": 4,
        "top": 2
    }

    rod_orientations = {
        "right": 1,
        "side": 2,
        "away": 3
    }

    available_actions = generate_actions(objects, holes, gear_orientations, rod_orientations)



    goal_test = goal_check(initial_state, ('inserted', "gear", "rod_1", "hole_1"))
    plan = astar(initial_state, goal_test, available_actions)
    

    print("Plan:")
    for step in plan:
        print(step)


if __name__ == '__main__':
    main()